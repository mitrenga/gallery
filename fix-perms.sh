#!/bin/bash
# Ensure the gallery/ and thumbs/ directories exist and have the ownership
# and permissions the web server needs (PHP-FPM and nginx run as www-data
# and must write thumbnails and .order.json, delete photos and serve files):
#
...
# refuse to touch a directory that is not a gallery project
if [ ! -f "$ROOT/getData.php" ]; then
    echo "not a gallery project (getData.php missing): $ROOT"; exit 1
fi
if [ "$(id -u)" -ne 0 ]; then
